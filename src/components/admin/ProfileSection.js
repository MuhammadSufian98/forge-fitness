...
export default function AccountProfileSection() {
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    setIsUpdating(true);
    const loadingToast = toast.loading("Synchronizing Terminal...");
    const result = await updateUserProfile(editForm);
    if (result.success) {
      toast.success("Terminal Synchronized Successfully!", {
        id: loadingToast,
      });
  ...
  const adminMetrics = [
    {
      label: "Active Hubs",
      value: user.hubAccess?.length || 8,
      icon: <Globe size={18} />,
    },
    { label: "System Uptime", value: "99.98%", icon: <Activity size={18} /> },
    {
      label: "Clearance Level",
      value: `LEVEL ${user.clearanceLevel || 10}`,
  ...
  const coachMetrics = [
    {
      ...
      value: user.accreditations?.length || 4,
      icon: <Award size={18} />,
    },
    {
      label: "Performance Rating",
      value: `${user.rating || 4.9}/5.0`,
      icon: <Zap size={18} />,
    },
    {
      label: "Retention Rate",
      value: `${user.retentionRate || 94}%`,
  ...
  const roleIcon = isAdmin ? (
    <ShieldCheck className="text-[#35a29f]" size={28} />
  ) : (
    <BadgeCheck className="text-[#35a29f]" size={28} />
  );
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              <div className="relative group">
                <div
                  className={`w-24 h-24 rounded-[2rem] ${isAdmin ? "bg-[#071952]" : "bg-[#088395]"} text-white flex items-center justify-center text-3xl font-black italic shadow-2xl overflow-hidden transition-transform group-hover:scale-95`}
                >
                  {user.profileImage ? (
                    <img
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    <h1 className="text-4xl font-black text-[#071952] uppercase italic tracking-tighter">
                      {user.fullName}{" "}
                      <span className={themeColor}>Profile</span>
                    </h1>
                    {roleIcon}
                  </div>
                  ...
                  <button
                    onClick={() => setIsEditModalOpen(true)}
                    className="px-5 py-2.5 bg-[#071952] text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-[#088395] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#071952]/20"
                  >
            ...
            {metrics.map((metric, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[9px] font-black text-[#071952]/30 uppercase tracking-widest">
                    {metric.label}
                  </span>
                </div>
                <p className="text-2xl font-black text-[#071952] italic">
                  {metric.value}
                </p>
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-[#071952]/30 uppercase tracking-widest">
                  {isCoach ? "Coaching Philosophy" : "Administrative Notes"}
                </span>
              </div>
              <p className="text-base font-bold text-[#071952] leading-relaxed italic">
                {isCoach
                  ? user.coachingPhilosophy || "No philosophy statement added."
                  : user.adminNotes || "No administrative notes recorded."}
              </p>

              {isCoach && user.accreditations?.length > 0 && (
                <div className="pt-6 border-t border-[#f2f3f6]">
                  <span className="text-[9px] font-black text-[#071952]/30 uppercase tracking-widest block mb-4">
                    Accreditations
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {user.accreditations.map((acc, i) => (
                      <span
                        key={i}
                        className="px-4 py-2 bg-[#f2f3f6] rounded-full text-[10px] font-black text-[#071952] uppercase tracking-wider"
                      >
                        {acc}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black">
                        {user.phoneNumber || user.phone || "UNLINKED"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
            ...
            >
              <header className="p-10 border-b border-[#f2f3f6] flex justify-between items-center">
                <h3 className="text-3xl font-black text-[#071952] uppercase italic tracking-tighter">
                  Terminal <span className="text-[#088395]">Configuration</span>
                </h3>
                <button
                  onClick={() => setIsEditModalOpen(false)}
                  className="p-3 hover:bg-[#f2f3f6] rounded-2xl transition-colors"
                >
                  <X size={28} />
                </button>
              </header>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          coachingPhilosophy: e.target.value,
                        })
                      }
