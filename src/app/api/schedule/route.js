...
async function handleGET(req) {
  try {
    ...
    if (!user) {
      ...
      const now = new Date();
      const formattedSchedules = publicSchedules.map((schedule) => {
        let effectiveStartTime = new Date(schedule.startTime);
        let effectiveEndTime = new Date(schedule.endTime);

        if (dateParam) {
          const targetDate = new Date(dateParam);
          effectiveStartTime.setFullYear(
            targetDate.getFullYear(),
            targetDate.getMonth(),
            targetDate.getDate(),
          );
          effectiveEndTime.setFullYear(
            targetDate.getFullYear(),
            targetDate.getMonth(),
            targetDate.getDate(),
          );
        ...
        return {
          ...
          coachNames:
            schedule.coaches?.map((coach) =>
              typeof coach === "object" ? coach.fullName : "Coach",
            ) || [],
        };
      });
    ...
    const schedules = await Schedule.find(query, projection)
    ...
    if (dateParam) {
      ...
      const formattedSchedules = schedules.map((schedule) => {
        const sessionBookings = dateBookings.filter(
          (b) => b.scheduleId.toString() === schedule._id.toString(),
        );
        ...
        if (schedule.isDaily) {
          ...
          if (dayDiff > 0)
            effectiveEndTime.setDate(effectiveEndTime.getDate() + dayDiff);
        }

        const isEnrolled = sessionBookings.some(
          (b) => b.userId.toString() === user.id,
        );
          ...
          currentStatus:
            now > effectiveEndTime
              ...
              : now >= effectiveStartTime
                ? "Live"
                : "Upcoming",
          coachNames: schedule.coaches.map((c) =>
            typeof c === "object" ? c.fullName : "Coach",
          ),
        };
      });
      ...
      const formattedSchedules = userBookings
        .map((booking) => {
    ...
    logError("schedule.get.failure", error);
    return ApiResponse({
      success: false,
...
async function handlePOST(req) {
  try {
    ...
    logError("schedule.post.failure", error);
    return ApiResponse({
      success: false,
      message: "Internal Server Error",
      status: 500,
...
export const GET = withApiLogging(handleGET, "/api/schedule");
export const POST = withApiLogging(handlePOST, "/api/schedule");
